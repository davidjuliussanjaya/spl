                    ...
                    <form action="{{ route('kategori.store') }}" method="POST" data-draft-key="spl:draft:category-create">
                        ...
                        <div class="mb-4">
                            ...
                            <select class="form-select @error('fakultas_id') is-invalid @enderror" id="fakultas_id" name="fakultas_id">
                                <option value="" @selected(old('fakultas_id') === null || old('fakultas_id') === '')>Umum</option>
                                @foreach($fakultasList as $fakultas)
                                    <option value="{{ $fakultas->id }}" @selected((string) old('fakultas_id') === (string) $fakultas->id)>{{ $fakultas->kode }} — {{ $fakultas->nama }}</option>
                                @endforeach
                            </select>
